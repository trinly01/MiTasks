<template>
  <q-dialog ref="dialog" @hide="onDialogHide" persistent>
    <div class="bg-white q-pa-md q-gutter-sm" style="min-width: 300px;">
      <div class="q-field__label text-weight-light text-caption">Assigned to</div>
      <div class="row">
        <q-chip v-for="(m, i) in members" :key="m.id" @mouseleave="hovering = null" @mouseover="hovering = m.id">
          <q-avatar :color="$randomLastNameColor(m.displayName)" text-color="white">{{ $getFirstTwoChars(m.displayName) }}</q-avatar>
          {{ m.displayName }}
          <q-btn
            @click="() => {
              members.splice(i, 1)
            }"
            v-show="hovering === m.id" color="grey" style="left: 8px;" round dense icon="close"
          size="xs"/>
        </q-chip>
      </div>
      <q-select
        ref="membersSelect"
        @input-value="searchUsers"
        use-input
        filled
        v-model="membersToAdd"
        multiple
        :options="options.map(u => {
            return {
              label: u.displayName,
              value: u.id,
              ...u
            }
          })"
        use-chips
        stack-label
        label="Add assignees"
        />
      <div class="row q-gutter-sm">
        <q-space></q-space>
        <q-btn flat label="cancel" v-close-popup />
        <q-btn color="primary" icon="save" label="Save" @click="save" />
      </div>
    </div>
  </q-dialog>
</template>

<script>
export default {
  props: ['modelValue', 'proj', 'board', 'card'],

  data () {
    return {
      hovering: null,
      options: [],
      members: [],
      origMembers: [],
      membersToAdd: [],
    }
  },

  async mounted () {
    await this.getAssignees()
  },

  emits: [
    // REQUIRED
    'ok', 'hide'
  ],

  computed: {
  },

  methods: {
    async searchUsers (name) {
      const query = this.$qs.stringify({
        _q: name,
        _where: {
          project_membership: this.proj.id
        },
      })
      const { data } = await this.$strapi.get('/users/?'+query)
      console.log(this.options)
      this.$refs.membersSelect.showPopup()
      return this.options = data
    },
    async getAssignees () {
      const query = this.$qs.stringify({
        assignments: this.card.id
      })
      const { data } = await this.$strapi.get('/users/?'+query)
      this.members = data.map(u => ({...u, label: u.displayName}))
      this.origMembers = [...this.members]
    },
    show () {
      this.$refs.dialog.show()
    },

    // following method is REQUIRED
    // (don't change its name --> "hide")
    hide () {
      this.$refs.dialog.hide()
    },

    onDialogHide () {
      // required to be emitted
      // when QDialog emits "hide" event
      this.$emit('hide')
    },

    async save () {
      // on OK, it is REQUIRED to
      // emit "ok" event (with optional payload)
      // before hiding the QDialog

      const memberIDsToAdd = this.membersToAdd.map(u => u.id)

      const { data }= await this.$strapi.put('/cards/'+this.card.id, {
        assignedTo: [
          ...this.members.map(u => u.id),
          ...memberIDsToAdd
        ]
      })

      console.log(data)

      if (this.membersToAdd.length) {
        const dd = await this.$strapi.post('/activities', {
          action: 'assignedTo',
          from: this.board.name,
          to: this.board.name,
          user: this.$global.user.user.id,
          info: JSON.stringify(data.assignedTo.filter(u => memberIDsToAdd.includes(u.id)).map(q => ({ displayName: q.displayName, id: q.id }))),
          project: this.proj.id,
          card: this.card.id
        })

        await this.$strapi.post('/projects/sendMail', {
          subject: `${this.card.name} - ${this.proj.name}`,
          from: this.$global.user.user.email,
          to: this.card.owner.email,
          cc: data.assignedTo.map(m => m.email).join(', '),
          system: this.proj.name,
          html: {
            firstName: '',
            message: `${this.$global.user.user.displayName} assigned ${JSON.parse(dd.data.info).map(m => m.displayName).join(', ')} to "${this.card.name}"`,
            url: {
              text: `Login to ${this.proj.name}`,
              link: `${this.$global.mailURL}/#/project/${this.proj.id}/${this.card.id}`
            }
          }
        })
      }

      const removedMembers = this.origMembers.map(q => ({ displayName: q.displayName, id: q.id })).filter(u => !this.members.map(m => m.id).includes(u.id))

      if (removedMembers.length) {
        const dd = await this.$strapi.post('/activities', {
          action: 'removedAssignment',
          from: this.board.name,
          to: this.board.name,
          user: this.$global.user.user.id,
          info: JSON.stringify(removedMembers),
          project: this.proj.id,
          card: this.card.id
        })
      }

      // console.log('assignedTo', data)

      this.$emit('ok', data.assignedTo)
      // or with payload: this.$emit('ok', { ... })

      // then hiding dialog
      this.hide()
    },

    onCancelClick () {
      // we just need to hide the dialog
      this.hide()
    }
  }
}
</script>